...
pub async fn deserialize(bstream: &mut BufStream<TcpStream>) -> Result<Value> {
    let deser = RespDeserializer::from_reader(bstream);

    let (val, _) = deser.deserialize().await?;
    Ok(val)
}
...
impl<'a> RespDeserializer<'a> {
    ...
    pub fn deserialize(
        mut self,
    ) -> Pin<Box<dyn Future<Output = Result<(Value, Self)>> + Send + 'a>> {
        let mut bytes: Vec<u8> = Vec::with_capacity(64);
        ...
        Box::pin(async {
            ...
            match first_byte {
                b'+' => {
                    ...
                    Ok((Value::SimpleString((&bytes[..len]).into()), self))
                }
                b':' => {
                    self.bstream.read_until(LF, &mut bytes).await?;
                ...
                b'$' => {
                    ...
                    Ok((Value::BulkString(bytes_.into()), self))
                }
                b'*' => {
                    self.bstream.read_until(LF, &mut bytes).await?;
                    ...
                    for _i in 0..array_len {
                        ...
                        let (elem, deser_) = self.deserialize().await?;
                        self = deser_;
                        elems.push(elem);
